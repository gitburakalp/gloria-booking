function setDatepickers(elem, lang) {
    $(elem).each(function (i) {
        ...
        if (ww < 767) {
            container = $this.closest('.search-panel-expand').find('.dates:nth-child(1)')
        }
        ...
        var datepicker = $this.dateRangePicker({
            ...
            beforeShowDay: i == 1 ? function(t)
            {
                ...
                var valid = !(t.getDate() <= checkinDay && t.getMonth() == checkinMonth - 1 && t.getFullYear() == checkinYear);  //disable saturday and sunday
                var _class = '';
                var _tooltip = valid ? '' : '';
        ...
        if (i == 0) {
            datepicker.eq(i).bind('datepicker-change', function (event, obj) {
                ...
                if ($this.val() != "") {
                    var newDate = moment(obj.value, format).add(3, 'days').format(format);
                    $('#inpCheckoutDate').data('dateRangePicker').setStart(newDate).open();
                }
            });
        }
    })
...
function setChildDatepickers(elem, lang) {
    $(elem).each(function () {
        ...
        var childStartDate = moment(`01.01.${dateYear.getFullYear() - 11}`, format).format(format);

        $this.dateRangePicker({
            language: lang,
            container: ww < 768 ? $this.closest('[data-hidden-child]') : $this.parent(),
            format: format,
            startOfWeek: startDay,
            autoClose: true,
            singleDate: true,
        ...
        if (value != "") {
            ...
            $(this).data('dateRangePicker').setStart(newDate);
        }
    });
}
...
function checkPlaceHolder() {
    $('.fake-placeholder').each(function () {
        $('input.form-control,textarea.form-control').each(function () {
            if ($(this).val() != "") {
                $(this).closest('.fake-placeholder').addClass('focused');
            } else {
                $(this).closest('.fake-placeholder').removeClass('focused');
            }
        });
    }); 
}
...
function checkModernInput() {
    ...
    $('.fake-placeholder').each(function () {
        ...
        $this.parent().siblings('[class*=col]').addClass('pt-3');
        ...
        $inpElem.on('blur', function () {
            ...
            isValid === true ? $this.removeClass('focused') : $this.addClass('focused');
        });
    });
}
...
$('.hotel-select__menu').each(function(){
    ...
    $(this).find('.hotel-select__menu-item').each(function(){
...
$('#checkAvailablity').each(function(){
    $(this).on('click',function(){
        ...
        $('.hotel-select__menu-item').each(function(){
            $(this).find('input[type=radio]').is(':checked') == true ? count++ : "";
        });
    ...
    $(this).on('input',function(){
        ...
        if (dataMin != undefined) {
            ...
            if ($this.val().length < dataMin) {
                $errorOwner != undefined ? $errorOwner.addClass(errorClass) : $this.addClass(errorClass);
            }
            else {
                $errorOwner != undefined ? $errorOwner.removeClass(errorClass) : $this.removeClass(errorClass);
            }
        }
    })
});
